...
fn render_frame<K: KnowledgeRenderer>(ecs: &mut EcsCtx,
    ...
    if changed {
        renderer.update_and_publish_game_window(*action_id, level_knowledge, position);
    }
}
...
pub fn animate_frame<K: KnowledgeRenderer>(ecs: &mut EcsCtx,
    ...
    *action_id += 1;
    ...
    render_frame(ecs, spatial_hash, entity_id, level_id, action_id, observer, renderer);
}
