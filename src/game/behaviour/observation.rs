...
pub fn simple_npc_shadowcast<K: KnowledgeRenderer>(child: BehaviourNodeIndex) -> BehaviourSwitch<K> {
    ...
    BehaviourSwitch::new_returning(move |input| {
        ...
        let eye = entity.copy_position().unwrap();
        let vision_distance = entity.copy_vision_distance().unwrap();
        let mut knowledge = entity.borrow_mut_simple_npc_knowledge().unwrap();
        ...
        shadowcast.observe(eye, input.spatial_hash, vision_distance,
                           level_knowledge, ActionEnv::new(input.ecs, input.action_id));
